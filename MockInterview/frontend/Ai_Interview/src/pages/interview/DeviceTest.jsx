  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        stopMeter = createMicMeter(stream, (rms)=>{ setLevel(rms); setOk(o=>({...o, mic:rms>0.02}))});
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="space-y-3">
          ...
          <div className="h-2 rounded bg-slate-200 overflow-hidden">
            <div className="h-full bg-black" style={{width: `${Math.min(100, level*300)}%`}}/>
          </div>
        </div>
        <div className="space-y-3">
          <div className="p-4 border rounded-xl">
            ...
            <button className="btn btn-outline mt-3" onClick={()=>{
              const a = new Audio("/beep.mp3"); a.play(); setOk(o=>({...o, spk:true}));
            }}>스피커 테스트</button>
          </div>
        </div>
      </div>
      <div className="flex justify-end">
        <button className="btn btn-primary" disabled={!(ok.cam&&ok.mic&&ok.spk&&ok.perm)}
